...
For example: python tfIdf_stem.py "C:\Users\chait\Desktop\Spring 2020 - courses\AIT - 726\Homeworks\Homework 2 Submission\part1\homework2_part1_stems"
...
The data is tokenized and stemmed. The control shifts to count_vector_func_tok function where the counts for each word in sentence is computed and returned.
...
'''
The below function tokenizes and stems a given sentence and returns the stemmed tokens and setences combining the stemmed tokens.
'''
...
'''
The main function takes the input as the path of the tweet folder, invokes the preprocessing, dataset preperation and training related functions. It recieves the metrics and writes the results in to results_part1_stem.txt file.
'''
def main(path):
    ...
    with open("results_part1_stem.txt","w") as f:
